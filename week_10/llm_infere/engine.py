...
class Engine:
    ...
    def step(self):
    #"""Run one decoding step for all active sequences."""
    # Group sequences by length for efficient batching
    seq_groups = self._group_sequences_by_length()
    
    for (seq_ids, seqs) in seq_groups.items():
        # Concatenate inputs and run model
        input_ids = torch.cat([seq.next_token for seq in seqs])
        logits = self.model(input_ids, self.block_manager)
        
        # Sample next tokens
        for seq, seq_logits in zip(seqs, logits):
            next_token = sample_token(seq_logits, seq.sampling_params)
            seq.append_token(next_token)
            
            if seq.is_finished():
                self.block_manager.free(seq.seq_id)
                del self.active_seqs[seq.seq_id]